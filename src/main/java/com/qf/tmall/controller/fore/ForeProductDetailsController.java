...
/**
 * @author ACI
 * @Title: ForeProductDetailsController
 * @ProjectName com.qf
 * @Description: TODO
...
public class ForeProductDetailsController extends BaseController {
    ...
    //转到前台天猫-产品详情页
    @ApiOperation(value = "转到前台天猫-产品详情页")
    ...
    public String goToPage(HttpSession session, Map<String, Object> map,
        ...
        List<Product> loveProductList = productService.getList(new Product().setProduct_category(new Category().setCategory_id(category_id)), new Byte[]{0, 2}, null, new PageUtil().setCount(3).setPageStart(i), null, null);
        ...
        return "fore/include/productDetailsPage";
    }


    @ResponseBody
    @GetMapping(value = "product/getdata")
    public Product getdatas(HttpServletRequest request) {
        logger.info("获取产品ID");
        String pid = (String) request.getSession().getAttribute("pid");
        Integer product_id = Integer.parseInt("21");
        logger.info("获取产品信息");
        Product product = productService.get(product_id);
    ...
    public String guessYouLike(@PathVariable("cid") Integer cid, @RequestParam Integer guessNumber) {
        ...
        List<Product> loveProductList = productService.getList(new Product().setProduct_category(new Category().setCategory_id(cid)), new Byte[]{0, 2}, null, new PageUtil().setCount(3).setPageStart(i), null, null);
        if (loveProductList != null) {
            logger.info("获取产品列表的相应的一张预览图片");
            for (Product loveProduct : loveProductList) {
                loveProduct.setSingleProductImageList(productImageService.getList(loveProduct.getProduct_id(), (byte) 0, new PageUtil(0, 1)));
